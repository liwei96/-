...
class building extends Base
{
	...
	$id=$res->id;
        $ll=['bid'=>$id,'huan'=>5,'jiao'=>5,'zhou'=>5,'shang'=>5,'all'=>1];
        Man::create($ll);
        if($res){
    ...
    public function editimgs($id){
        ...
        $z_imgs=Zhu::where('bid',$id)->select();
        $t_imgs=Zimg::where('bid',$id)->select();
            
            // return;
        return view('editimgs',['id'=>$id,'j_imgs'=>$j_imgs,'h_imgs'=>$h_imgs,'x_imgs'=>$x_imgs,'p_imgs'=>$p_imgs,'s_imgs'=>$s_imgs,'y_imgs'=>$y_imgs,'j_imgs'=>$j_imgs,'z_imgs'=>$z_imgs,'t_imgs'=>$t_imgs]);
    }
    ...
    public function delpics(){
        ...
        if($data['type']=='x'){
            ...
            unlink(ROOT_PATH.DS.'public'.$x['x_small']);
            unlink(ROOT_PATH.DS.'public'.$x['x_big']);
            Xiaoimgs::destroy($id);
        }else if($data['type']=='y'){
            $x=Yangimgs::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['y_small']);
            unlink(ROOT_PATH.DS.'public'.$x['y_big']);
            Yangimgs::destroy($id);
        }else if($data['type']=='p'){
            $x=Peiimgs::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['p_small']);
            unlink(ROOT_PATH.DS.'public'.$x['p_big']);
            Peiimgs::destroy($id);
        }else if($data['type']=='s'){
            $x=Shiimgs::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['s_small']);
            unlink(ROOT_PATH.DS.'public'.$x['s_big']);
            Shiimgs::destroy($id);
        }else if($data['type']=='j'){
            $x=Jiaoimgs::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['j_small']);
            unlink(ROOT_PATH.DS.'public'.$x['j_big']);
            Jiaoimgs::destroy($id);
        }else if($data['type']=='h'){
            $x=Huimgs::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['h_small']);
            unlink(ROOT_PATH.DS.'public'.$x['h_big']);
            Huimgs::destroy($id);
        }else if($data['type']=='t'){
            $x=Zimg::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['img']);
            Zimg::destroy($id);
        }else if($data['type']=='z'){
            $x=Zhu::where('id',$id)->find();
            unlink(ROOT_PATH.DS.'public'.$x['img']);
            Zhu::destroy($id);
        }
        $res=[
    ...
    public function upload_t($bid)
    {
        $flies = request()->file('t_pics');
        $pics_data = [];
        foreach ($flies as $file) {
            $info = $file->validate(['size' => 3 * 1024 * 1024, 'ext' => 'jpg,gif,jpeg,png'])->move(ROOT_PATH . 'public' . DS . 'uploads');
            if ($info) {
                ...
                $pics_big = DS . 'uploads' . DS . $temp[0] . DS . 'thumb_800_' . $temp[1];
                $image = Image::open('.' . $pics_origin);
                $image->thumb(2000, 800)->save('.' . $pics_big);
                $row = [
                    'bid' => $bid,
                    'img' => $pics_big,
                ];
    ...
    public function upload_z($bid)
    {
        $flies = request()->file('z_pics');
        $pics_data = [];
        foreach ($flies as $file) {
            $info = $file->validate(['size' => 3 * 1024 * 1024, 'ext' => 'jpg,gif,jpeg,png'])->move(ROOT_PATH . 'public' . DS . 'uploads');
            if ($info) {
